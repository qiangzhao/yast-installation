  ...
  private
    ...
    def valid_repositories?
      ...
      if ENV["Y2_FORCE_SELF_UPDATE"] == "1"
        ...
        repos = update_repositories.join("\n")
        # TRANSLATORS: error message, %{repos} is replaced by an URL list
        Report.Error(_("The installer found unexpected package versions in \n" \
          "the installer update repository. Make sure that the self update\n" \
